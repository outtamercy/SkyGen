...
import time
import configparser
import re # Ensure this line is at the very top of the file
...
def run_xedit_export(organizer: mobase.IOrganizer, xedit_exe_path: Path, xedit_script_path: Path, mo2_exec_name_to_use: str, target_plugin_name: str, game_version: str, output_base_dir: Path, dialog) -> Path | bool:
    ...
    # NEW: Create a super-sanitized stem for the Pascal debug log filename
    ...
    super_sanitized_stem = re.sub(r'[^\w]', '', Path(target_plugin_name).stem)
    
    # This sanitizes for Python/OS compatibility, but the super_sanitized_stem is for Pascal.
    sanitized_target_plugin_stem = Path(target_plugin_name).stem.replace(' ', '_').replace("'", "")

    export_json_path = output_base_dir / f"SkyGen_xEdit_Export_{sanitized_target_plugin_stem}_{timestamp}.json"
    export_log_path = output_base_dir / f"SkyGen_xEdit_Log_{sanitized_target_plugin_stem}_{timestamp}.log"
    
    # Define the path for the Pascal script's debug log using the super-sanitized stem
    export_pascal_debug_log_path = output_base_dir / f"ExportPluginData_Debug_{super_sanitized_stem}_{timestamp}.log"

    # Sanitize target_plugin_name for passing to Pascal script via -D:TargetPlugin
    sanitized_plugin_name_for_script = target_plugin_name.replace("'", "").replace("&", "and")
    ...
    xedit_args = [
        ...
        f"-o:\"{os.path.normpath(str(export_json_path))}\"",
        f"-l:\"{os.path.normpath(str(export_log_path))}\"",
        f"-debuglog:\"{os.path.normpath(str(export_pascal_debug_log_path))}\"", # Correctly passing Pascal debug log path
    ...
    organizer.log(0, f"SkyGen: DEBUG: Launching xEdit with CWD: {cwd}") # Debug log for startApplication (removed stdout/stderr)
    organizer.log(0, f"SkyGen: DEBUG: Pascal script debug log path passed: {export_pascal_debug_log_path}")
    
    try:
        # Removed stdout=True, stderr=True as they are not supported by your MO2 version
        app_handle = organizer.startApplication(mo2_exec_name_to_use, xedit_args, str(cwd))

        if app_handle == 0:
            dialog.showError("xEdit Launch Failed", f"Failed to launch '{mo2_exec_name_to_use}' via MO2. Check your MO2 setup for this executable and MO2 logs.")
            organizer.log(3, f"MO2 startApplication failed to launch xEdit executable '{mo2_exec_name_to_use}'.")
            return False
        ...
        pass # No cleanup needed for temp_loadorder_path anymore
