# __init__.py (Note: This is not the __init__.py for the package, but the plugin's main file)
...
class SkyGenGeneratorTool(mobase.IPluginTool):
    ...
    def init(self, organizer: mobase.IOrganizer):
        ...
        self.wrapped_organizer.set_log_file_path(Path(self.organizer.pluginDataPath()) / "SkyGen" / "skygen_debug.log") # Set custom log file
    ...
    def display(self):
        ...
        # This temp_msg_box is a dummy as we only need the dialog instance's error method later
        ...
        if config_file_path.is_file():
            try:
                ...
                self.wrapped_organizer.log(3, f"SkyGen: ERROR: Could not load config.json for path lookup: {e}")
        ...
        self.xedit_exe_path, self.xedit_mo2_name = get_xedit_exe_path(
            config_data_for_path_lookup, 
            self.wrapped_organizer, # <--- PASS self.wrapped_organizer
            self.dialog # Pass the dialog instance for error messages
        )
        ...
        self.game_root_path = get_game_root_from_general_ini(
            self.wrapped_organizer, # <--- PASS self.wrapped_organizer
            config_data_for_path_lookup # Pass config_data_for_path_lookup here
        )
            ...
            if selected_output_type == "SkyPatcher YAML":
                ...
                if self.dialog.generate_all:
                    ...
                    if not target_plugin_filename:
                        ...
                        clean_temp_script_and_ini(self.xedit_exe_path, Path(self.organizer.pluginDataPath()) / "SkyGen" / "ExportPluginData.pas", self.wrapped_organizer) # Ensure cleanup
                        return
                    ...
                    if not xedit_output_path_target_all:
                        ...
                        clean_temp_script_and_ini(self.xedit_exe_path, xedit_script_path, self.wrapped_organizer) # Ensure cleanup
                        return
                    ...
                    clean_temp_script_and_ini(self.xedit_exe_path, xedit_script_path, self.wrapped_organizer) # Clean up after target export
                    ...
                    for current_source_mod_display_name in source_mods_for_all:
                        ...
                        if xedit_output_path_source:
                            source_exported_json = load_json_data(self.wrapped_organizer, xedit_output_path_source, f"xEdit Export for {current_source_mod_display_name}", self.dialog)
                            clean_temp_script_and_ini(self.xedit_exe_path, xedit_script_path, self.wrapped_organizer) # Clean up after source export
                    ...
                    source_exported_json = load_json_data(self.wrapped_organizer, xedit_output_path_source, "Source Mod xEdit Export", self.dialog)
                    clean_temp_script_and_ini(self.xedit_exe_path, xedit_script_path, self.wrapped_organizer) # Clean up after source export
                ...
                igpc_data = load_json_data(self.wrapped_organizer, igpc_json_path, "IGPC JSON", self.dialog)
                if igpc_data:
                    generate_bos_ini_files(self.wrapped_organizer, igpc_data, output_folder_path, self.dialog)
                else:
