...
def write_xedit_ini_for_skygen(xedit_exe_path: Path, wrapped_organizer): # ADD wrapped_organizer parameter
...
ShowSplashScreen=False"""
    try:
        ...
        wrapped_organizer.log(1, f"SkyGen: Successfully wrote xEdit INI to: {ini_path}") # ADDED SUCCESS LOG
    except Exception as e:
            # Changed to use wrapped_organizer.log for consistency
            wrapped_organizer.log(3, f"SkyGen: ERROR: Failed to write xEdit INI: {e}")
...
def run_xedit_export(
    ...
    try:
        ...
        write_xedit_ini_for_skygen(xedit_exe_path, wrapped_organizer) # Pass wrapped_organizer
        ...
        for subdir in xedit_subdirs_to_create:
            try:
                ...
                wrapped_organizer.log(1, f"SkyGen: Ensured xEdit subdirectory exists: {subdir}") # Added success log
            except Exception as e:
                # Changed to ERROR level and added a dialog pop-up to make failure immediately visible
