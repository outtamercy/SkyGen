...
# a. Adjust run_xedit_export function signature:
def run_xedit_export(organizer: mobase.IOrganizer, xedit_exe_path: Path, xedit_script_path: Path,
                     mo2_exec_name_to_use: str, target_plugin_filename: str, game_version: str, # Renamed target_plugin_name to target_plugin_filename
                     output_base_dir: Path, dialog, target_mod_display_name: str) -> Path | bool: # Added target_mod_display_name
    organizer.log(1, f"Preparing to run xEdit export for '{target_plugin_filename}' in DyneLOD style...")

    # Ensure xEdit executable exists
    if not xedit_exe_path.is_file():
        dialog.showError("xEdit Not Found", f"xEdit executable not found at {xedit_exe_path}.")
        return False

    # The xedit_script_path is now expected to be the final, correctly staged path from plugin.py
    # c. Delete the temporary copying logic from run_xedit_export:
    # This entire block has been removed.

    if not xedit_script_path.is_file():
        dialog.showError("Script Not Found", f"xEdit script not found at the provided path: {xedit_script_path}.")
        return False
    ...
    try:
        ...
        dialog.showError("Output Directory Error", f"Could not create output directory {output_base_dir}:\n{e}")
        return False

    # Create a unique output path for the export file
    timestamp = int(time.time())
    sanitized_plugin_name_for_filename = sanitize_path_for_pascal(target_plugin_filename).replace('.', '')
    export_json_path = output_base_dir / f"SkyGen_xEdit_Export_{sanitized_plugin_name_for_filename}_{timestamp}.json"
    # d. Correct export_pascal_debug_log_path and add export_log_path definition:
    export_log_path = output_base_dir / f"SkyGen_xEdit_Log_{sanitized_plugin_name_for_filename}_{timestamp}.log" # NEW: Define export_log_path
    pascal_debug_log_path_for_args = output_base_dir / f"ExportPluginData_Debug_{sanitized_plugin_name_for_filename}_{timestamp}.log" # NEW: Define for args
            

    # Build xEdit command-line arguments in DyneLOD style.
    # Insert the game mode flag, and include -autoload so xEdit loads the active mods dynamically.
    xedit_args = []
    game_mode_arg = {"SkyrimSE": "-sse", "SkyrimVR": "-tes5vr"}.get(game_version)
    if game_mode_arg:
        xedit_args.append(game_mode_arg)
    else:
        organizer.log(2, f"No game mode flag for {game_version} -- launching without explicit game mode.")

    # e. Modify argument construction for xEdit (xedit_args):
    xedit_args.append(f'-script:"{os.path.normpath(str(xedit_script_path))}"') # Our script
    xedit_args.append(f'-o:"{export_json_path}"') # Output JSON
    xedit_args.append(f'-l:"{export_log_path}"') # Output Log
    xedit_args.append(f'-debuglog:"{pascal_debug_log_path_for_args}"') # Pascal debug log

    xedit_args.append(f'-plugin:"{target_plugin_filename}"') # Explicitly load the target plugin
    xedit_args.append(f'-D:TargetPlugin="{target_mod_display_name}"') # Pass display name to Pascal script

    xedit_args.extend([
        '-IKnowWhatImDoing',
        '-NoAutoUpdate',
        '-NoAutoBackup',
        f'-b:"{os.path.normpath(str(output_base_dir / "Backup"))}"',
        '-autoload',
        '-nomenus',
        '-exit'
    ])

    organizer.log(0, f"DEBUG: Final xEdit command: {' '.join(xedit_args)}")
    organizer.log(0, f"DEBUG: Export JSON path: {export_json_path}")

    # Determine the working directory: prioritize game root from ModOrganizer.ini, then fallback to MO2 base.
    cwd_path = get_game_root_from_general_ini(organizer.basePath(), organizer, dialog)
    if not cwd_path:
        cwd_path = Path(organizer.basePath()) # Fallback to MO2's base path
        organizer.log(2, f"SkyGen: WARNING: Falling back to MO2 base path as CWD: {cwd_path}")
    
    # Final check and normalization for cwd
    if not cwd_path or not cwd_path.exists():
        # As a last resort, use the xEdit executable's directory
        cwd_path = xedit_exe_path.parent
        organizer.log(2, f"SkyGen: WARNING: Game root/MO2 base not valid, falling back to xEdit directory as CWD: {cwd_path}")

    cwd = os.path.normpath(str(cwd_path))
    organizer.log(0, f"DEBUG: Final working directory for xEdit: {cwd}")
    ...
    try:
        ...
        # Poll for creation of the export file.
        ...
        while elapsed < max_poll_time:
            ...
            organizer.log(0, f"Waiting for xEdit export ({elapsed}/{max_poll_time}s)... Output: {export_json_path.exists()}, Size: {export_json_path.stat().st_size if export_json_path.exists() else 'N/A'}")
            time.sleep(1)
            elapsed += 1

        dialog.showError("xEdit Timeout", f"xEdit did not produce the output file '{export_json_path.name}' within {max_poll_time} seconds. Check xEdit logs for errors.")
        return False

    except Exception as e:
        organizer.log(3, f"Error launching or running xEdit: {e}\n{traceback.format_exc()}")
        dialog.showError("xEdit Error", f"An error occurred while launching xEdit:\n{e}. Check MO2 logs for more details.")
        return False
    finally:
        # Corrected: Use pascal_debug_log_path_for_args which is in output_base_dir
        export_pascal_debug_log_path = pascal_debug_log_path_for_args 
...
def generate_bos_ini_files(organizer: mobase.IOrganizer, igpc_data: dict, output_base_dir: Path, dialog_instance) -> bool:
    ...
    for record in igpc_records:
        ...
        if is_disabled == 1:
            ...
            rot_z = record.get("RotZ") # Corrected from rot_z to RotZ
