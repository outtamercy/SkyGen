...
def run_xedit_export(organizer: mobase.IOrganizer, xedit_exe_path: Path, xedit_script_path: Path, mo2_exec_name_to_use: str, target_plugin_name: str, game_version: str, output_base_dir: Path, dialog) -> Path | bool:
    ...
    super_sanitized_filename_base = re.sub(r'[^\w]', '', Path(target_plugin_name).stem)

    export_json_path = output_base_dir / f"SkyGen_xEdit_Export_{super_sanitized_filename_base}_{timestamp}.json"
    export_log_path = output_base_dir / f"SkyGen_xEdit_Log_{super_sanitized_filename_base}_{timestamp}.log"
    
    # Define the path for the Pascal script's debug log using the same super-sanitized stem
    export_pascal_debug_log_path = output_base_dir / f"ExportPluginData_Debug_{super_sanitized_filename_base}_{timestamp}.log"
